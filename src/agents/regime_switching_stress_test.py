...
fixed at 0.5 but instead changes at random regime boundaries:
    ...
    - Regime duration:  L ~ Pareto(α=3/2, L_min=10)   (in MO events)
    - p_buy per regime: U[p_lo, p_hi]                    (default [0.2, 0.8])

The Pareto distribution for regime durations is motivated by the empirically
observed power-law tails in metaorder durations (Lillo, Mike & Farmer 2005).
...
# / comparison_*_by_throttle.py.  Viridis cycler gives distinct hues that
# also order perceptually, so multi-line plots in the regime-switching
# section share the same look-and-feel as the rest of the paper. ===
plt.rcParams.update({
    "axes.labelsize": 15,
    "axes.titlesize": 14,
    "xtick.labelsize": 13,
...
# ── Monte Carlo ─────────────────────────────────────────────────────────────
N_SIMS = 1000          # number of independent simulation episodes per condition
...
DQN_CHECKPOINT = "checkpoints/deep_mm_mtm_pure_invp0.0010_final.pt"
...
# ── AS misspecified overlay (Expected PnL vs P(buy MO)) ─────────────────────
# When enabled, this script enters an asymmetry-sweep workflow and plots:
#   - DQN empirical curve (blue)
#   - AS misspecified analytical curve (red)
# The AS curve assumes the MM quotes symmetrically while the environment has
# asymmetric MO flow (p != 0.5), which induces inventory-drift losses.
#
# Priority rule:
#   - If PLOT_ONLY_AS_MISS=True  -> plot ONLY the AS red curve.
#   - Else if PLOT_AS_MISS=True  -> plot both DQN (blue) + AS (red).
...
# Gamma requested for the AS misspecified theoretical curve.
AS_MISS_GAMMA = 1e-6
...
# Calibration steps for AS miss curve — use a long simulation (500k) for
...
AS_MISS_A_CACHED     = None    # execution intensity intercept
AS_MISS_KAPPA_CACHED = None     # execution intensity decay rate
...
AS_MISS_V0_CACHED    = None        # set after first calibration run
...
AS_MISS_R_FILL_FACTOR = None       # will be measured on first run

# ── Regime-switching parameters ─────────────────────────────────────────────
# These control the non-stationary MO flow environment.
# REGIME_ALPHA: Pareto shape — higher values produce shorter, less variable
#   regimes; 1.5 matches empirical metaorder duration distributions.
# REGIME_L_MIN: minimum regime length in MO events — sets the Pareto scale.
#   With alpha=1.5, E[L] = alpha * L_min / (alpha - 1) = 3 * L_min.
...
# ═══════════════════════════════════════════════════════════════════════════════
# AS MISSPECIFIED CURVE (Expected PnL vs P(buy MO))
# ═══════════════════════════════════════════════════════════════════════════════
...
def estimate_episode_time_horizon(seed: int = SEED_CALIB + 2) -> float:
    ...
    distort AS formulas that are expressed in continuous-time intensities.
    """
    if USE_QRM:
        print("[AS MISS] NOTE: T estimation uses Santa-Fe simulator tape as proxy under USE_QRM.")
    ...
    if "Time" not in msg_df.columns or len(msg_df) < 2:
        print("[AS MISS] WARNING: could not infer Time from tape; falling back to T=1.0")
        return 1.0

    t0 = float(msg_df["Time"].iloc[0])
    t1 = float(msg_df["Time"].iloc[-1])
    T = max(t1 - t0, 1e-9)
    print(f"[AS MISS] Episode time horizon estimated: T={T:.6f}")
    return T
...
def calibrate_v0(
    ...
    """
    print(f"[AS MISS] Calibrating v0 from bare LOB simulations...")
...
def calibrate_r_fill_factor(
    ...
    The theoretical fill rate R = A * exp(-k * delta_bar) assumes the MM
    captures ALL fills at that price level with continuous quoting.  In
    practice, the MM shares the queue and re-quotes at discrete intervals.
    ...
    because the analytical curve models an AS/GLFT MM, not a DQN.
    ...
    """
    # A is calibrated from ONE side (bid only, side_mode="buy").
    # R_measured counts fills on BOTH sides.  To compare apples-to-apples,
    # R_theoretical must also be both-sided: 2 * A * exp(-k * delta_bar).
    R_theoretical = 2.0 * float(A * np.exp(-kappa * delta_bar))

    print(f"[AS MISS] Calibrating R_FILL_FACTOR from {n_episodes} GLFT episodes at p=0.5...")
...
def compute_as_miss_expected_pnl_curve(
    ...
    Model:
        1) GLFT asymptotic quotes with inventory skew (p.11):
           ...
           where c = sqrt(σ²γ / (2kA)) · (1 + γ/k)^(1 + k/γ)

        2) Fill rates under true asymmetry + inventory skew:

               r_a(q) = p · A · exp(-k · δ_a(q))
               r_b(q) = (1-p) · A · exp(-k · δ_b(q))
    ...
    A, kappa : float
        ...
        A is per-side intensity (calibrated from buy-side).
    sigma : float
    ...
    #
    # Numerical stability: (1 + γ/k)^(1 + k/γ) can overflow for small γ.
    # Use log-form: exp((1 + k/γ) · ln(1 + γ/k)).
    # As γ → 0: the exponent → 1, so the power term → e ≈ 2.718.
    if gamma > 1e-12:
        _log_base = (1.0 + kappa / gamma) * np.log1p(gamma / kappa)
    ...
    for p in probs:
        ...
        r_a = p * A * np.exp(-kappa * delta_a) * r_fill_factor
        r_b = (1.0 - p) * A * np.exp(-kappa * delta_b) * r_fill_factor
    ...
    meta = {
        ...
        "c_skew": float(c_skew),
        "R_at_q0": float(A * np.exp(-kappa * delta_base) * r_fill_factor),
...
def plot_expected_pnl_as_miss_overlay(
    ...
    Plot Expected PnL vs P(buy MO):
      ...
      - AS misspecified analytical curve in red
    ...
    if (not only_as) and dqn_means is not None:
        ...
        ax.errorbar(
            ...
            fmt="o-", color="royalblue", capsize=4, linewidth=2, markersize=7,
            label="DQN Rainbow (empírico)",
        )
    ...
    if glft_means is not None:
        ...
        ax.errorbar(
            ...
            fmt="s-", color="forestgreen", capsize=4, linewidth=2, markersize=6,
            label=f"GLFT (γ={GLFT_GAMMA:.1e}, empírico)",
        )

    ax.plot(
        x, np.asarray(as_means, dtype=float),
        "o-", color="crimson", linewidth=2, markersize=6,
        label="AS misspecified (analítico)",
    )
    ...
    # Title depends on what's plotted
    _curves = []
    if (not only_as) and dqn_means is not None:
        _curves.append("DQN")
    if glft_means is not None:
    ...
    _curves.append("AS")
    ax.set_title(
        "Expected PnL vs MO Flow Asymmetry"
        + f"\n({' vs '.join(_curves)})",
        fontsize=13,
    )
    ...
    _v0_str = f", v₀={as_meta['v0']:.4g}" if as_meta.get('use_v0') else ""
    txt = (
        f"A={as_meta['A']:.3g}, k={as_meta['kappa']:.3g}, σ={as_meta.get('sigma', 0):.3g}\n"
        f"γ={as_meta.get('gamma', 0):.1e}, c_skew={as_meta.get('c_skew', 0):.4g}{_v0_str}\n"
        f"T={as_meta['T']:.3g}, Q={as_meta.get('inv_limit', '?')}, "
        f"η={as_meta.get('r_fill_factor', 1):.3f}"
    )
    ax.text(
        0.02, 0.02, txt, transform=ax.transAxes, fontsize=9,
...
def run_dqn_sims(buy_mo_prob, ctrl, label: str = "") -> np.ndarray:
    ...
    for i in range(N_SIMS):
        ...
        if buy_mo_prob == "regime":
            bmp = make_regime_schedule(seed=seed + 999_999, n_mo_events=N_STEPS)
        else:
            bmp = buy_mo_prob
...
def plot_regime_example(seed: int = 42, n_mo_events: int = 5000) -> None:
    ...
    to help calibrate the regime parameters (L_min, alpha).
    """

    sched = make_regime_schedule(seed=seed, n_mo_events=n_mo_events)
    ...
    ax.set_title(
        f"Example Regime Schedule  "
        f"(α={REGIME_ALPHA}, L_min={REGIME_L_MIN}, seed={seed})",
        fontsize=12,
    )
    ax.set_ylim(0.1, 0.9)
    ax.legend(fontsize=10)
    ...
    print(f"  Mean length: {np.mean(lengths):.1f} events")
    print(f"  Median length: {np.median(lengths):.1f} events")
    print(f"  Max length: {np.max(lengths)} events")
...
def plot_comparison(
    ...
    fig.suptitle(
        ...
        f"α={REGIME_ALPHA}, L_min={REGIME_L_MIN})",
        fontsize=14, y=1.02,
    )
    plt.tight_layout()
...
# ═══════════════════════════════════════════════════════════════════════════════
# AS MISSPECIFIED WORKFLOW
# ═══════════════════════════════════════════════════════════════════════════════

def run_as_miss_plot_workflow() -> None:
    """
    Run the asymmetry sweep workflow requested for AS misspecified analysis.

    Modes:
      - PLOT_AS_MISS=True,  PLOT_ONLY_AS_MISS=False:
          plot DQN empirical (blue) + AS miss analytical (red).
      - PLOT_ONLY_AS_MISS=True:
          plot only AS miss analytical (red).
    """
    only_as = bool(PLOT_ONLY_AS_MISS)

    print("\n=== AS MISSPECIFIED CURVE MODE ===")
    ...
    if AS_MISS_A_CACHED is not None and AS_MISS_KAPPA_CACHED is not None:
        ...
        print(f"[AS MISS] Using cached calibration: A={A:.6f}, kappa={KAPPA:.5f}")
    else:
    ...
    # Compute delta_bar for R_FILL_FACTOR calibration
    if float(AS_MISS_GAMMA) > 1e-12:
        _delta_spread = np.log1p(float(AS_MISS_GAMMA) / KAPPA) / float(AS_MISS_GAMMA)
    else:
        _delta_spread = 1.0 / KAPPA
    _delta_bar = float(_delta_spread)  # asymptotic GLFT (no time-dependent term)

    # Calibrate R_FILL_FACTOR using GLFT (not DQN) at p=0.5.
    # The analytical curve models an AS/GLFT MM, so we measure the fill
    ...
    if AS_MISS_R_FILL_FACTOR is not None:
        ...
        print(f"[AS MISS] Using cached R_FILL_FACTOR = {r_fill_factor:.6f}")
    else:
    ...
    if AS_MISS_USE_V0:
        if AS_MISS_V0_CACHED is not None:
            ...
            print(f"[AS MISS] Using cached v0 = {_v0:.6f}")
        else:
            _v0 = calibrate_v0()
